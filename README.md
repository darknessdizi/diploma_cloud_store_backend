...
После копирования проекта с помощью `git clone` (пункт 1 на рисунке) на сервере появится папка с проектом (пункт 2 на рисунке). Список папок можно получить командой `ls`, `ls -A` или `ls --all`

14. Перейти в появившуюся папку `diploma_cloud_store_backend`:
```bash
cd backend/
```
